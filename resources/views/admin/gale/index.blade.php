...
@section('content')
	...
	<div class="mt-16">
		<a href="{{ route('gale.create' )}}" class="bg-blue-dark hover:bg-blue px-4 text-white text-sm py-2 rounded-full ml-12 mr-2 no-underline" >Create a Category</a>
	          ...
	          <tr class="border-b border-t text-grey-darker text-sm">
	            ...
	            <td class="py-4 pl-6">{{$category->name}}</td>
	                <td class="flex justify-end"> 
	                  <a href="{{ route('gale.show', $category->id )}}" class="text-grey-dark pt-5 pr-2 pl-2" title="view"><i class="fa fa-eye " ></i></a>
	                   <a href="{{ route('gale.edit', $category->id )}}" class="text-grey-dark pt-5 pr-2 pl-2"><i class="fa fa-pencil-square-o" title="edit"></i></a>
	                   <form action="{{ route('gale.destroy', $category->id)}}" method="POST">
