...
<div class="mt-16">
	<a href="{{ route('upevent.create' )}}" class="bg-blue-dark hover:bg-blue px-4 text-white text-sm py-2 rounded-full ml-12 mr-2 no-underline" >Create Event</a>
	          ...
	          <tr class="border-b border-t text-grey-darker text-sm">
	            ...
	            <td class="py-4 pl-6">{{$upevent->content}}</td>
	                <td class="flex justify-end"> 
	                  <a href="{{ route('upevent.show', $upevent->id )}}" class="text-grey-dark pt-5 pr-2 pl-2" title="view"><i class="fa fa-eye " ></i></a>
	                   <a href="{{ route('upevent.edit', $upevent->id )}}" class="text-grey-dark pt-5 pr-2 pl-2"><i class="fa fa-pencil-square-o" title="edit"></i></a>
	                   <form action="{{ route('upevent.destroy', $upevent->id)}}" method="POST">
